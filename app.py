...
app.config['SQLALCHEMY_DATABASE_URI']="sqlite:///mentordatabase.db"
app.config['SQLALCHEMY_BINDS']={'menteedatabase':"sqlite:///menteedatabase.db"}
app.config['SQLALCHEMY_TRACK_MODIFICATIONS']=False
db=SQLAlchemy(app)

#Mentor DETAILS TABLE
class mentordatabase(db.Model): #table column defination
...
class menteedatabase(db.Model):
    __bind_key__ = 'menteedatabase'
...
def MentorDetailsPage():
    if request.method=='POST':
        ...
        insert = mentordatabase(name=name,email=email,pronouns=pronouns,AD=AD,AI=AI,coding=coding,CS=CS,CyS=CyS,DS=DS,IT=IT,IOS=IOS,robotics=robotics,SE=SE,UI=UI,WD=WD,dropdownMenuButton1=dropdownMenuButton1,Bio=Bio,Availability=Availability)
        db.session.add(insert)
        db.session.commit()
    allMentorDetails=mentordatabase.query.all()
    list()
...
def MenteeDetailsPage():
    if request.method=='POST':
        ...
        insert = menteedatabase(name2=name2,email2=email2,pronouns2=pronouns2,AD2=AD2,AI2=AI2,coding2=coding2,CS2=CS2,CyS2=CyS2,DS2=DS2,IT2=IT2,IOS2=IOS2,robotics2=robotics2,SE2=SE2,UI2=UI2,WD2=WD2,dropdownMenuButton2=dropdownMenuButton2,goals2=goals2,Bio2=Bio2,Availability2=Availability2)

        db.session.add(insert)
        db.session.commit()
        
    allMenteeDetails=menteedatabase.query.all()
...
def MentorResult():
    if request.method=='POST':
        ...
        if username=='geshita' and password == 'geshita':
            ...
            con = sql.connect("mentordatabase.db")
            con.row_factory = sql.Row
            cur = con.cursor()
            cur.execute("select * from mentordatabase")
            rows = cur.fetchall();
...
def MenteeResult():
    if request.method=='POST':
        ...
        password=request.form['password']
        if username=='geshita' and password == 'geshita':
            con = sql.connect("menteedatabase.db")
            con.row_factory = sql.Row
            
            cur = con.cursor()
            cur.execute("select * from menteedatabase")
